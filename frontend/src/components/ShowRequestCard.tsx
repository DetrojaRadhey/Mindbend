...
export default function ShowRequestCard() {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_BACKEND_API}/request/my-requests`, {
  ...
  const handleCancelRequest = async (requestId: string) => {
    try {
      const response = await axios.delete(`${import.meta.env.VITE_BACKEND_API}/request/delete/${requestId}`, {
        withCredentials: true,
        headers: {
          'Content-Type': 'application/json'
        }
