...
const ServiceProviderDashboard = () => {
  ...
  const fetchPendingRequests = async () => {
    try {
      setIsLoading(true);
      setError(null);
      const response = await axios.get(`${import.meta.env.VITE_BACKEND_API}/request/provider/requests`, {
  ...
  const fetchAcceptedServiceRequests = async () => {
    try {
      setIsLoading(true);
      setError(null);
      const response = await axios.get(`${import.meta.env.VITE_BACKEND_API}/request/checkacceptedrequest`, {
        withCredentials: true
      });
  ...
  const handleCloseRequest = async (id: string, isEmergency: boolean) => {
    try {
      if (isEmergency) {
        ...
        const response = await axios.post(`${import.meta.env.VITE_BACKEND_API}/request/complete-request`, {
          requestId: id
        }, {
          withCredentials: true
        });
                    ...
                    <Button 
                      ...
                      onClick={async () => {
                        try {
                          ...
                          const response = await axios.post(`${import.meta.env.VITE_BACKEND_API}/request/accept-request`, {
